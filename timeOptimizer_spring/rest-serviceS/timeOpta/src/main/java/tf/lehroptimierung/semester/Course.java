...
public class Course {
    ...
    double calcWeightedAverage(double timeContent, double timeDidactic, double timePresentation) {
       ...
       // System.out.println(" ---------------WEIGHTS--------------------" + content.getWeight() + " " + didactic.getWeight() + " " + presentation.getWeight());
        double totalWeight = content.getWeight() + didactic.getWeight() + presentation.getWeight();

        double WAM = (weightedContent + weightedDidactic + weightedPresentation) / totalWeight;
        return WAM;
    }
}
