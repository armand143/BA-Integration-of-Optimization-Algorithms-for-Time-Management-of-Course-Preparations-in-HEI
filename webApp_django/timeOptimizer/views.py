        ...
        if request.method == 'POST':
            ...
            if 'updateLecturer-submit' in request.POST:
                ...
                if updateLecturerForm.is_valid():
                    ...
                    picked_semester = updateLecturerForm.cleaned_data['picked_semester'].first()
                    if picked_semester is not None:
                        currentLecturer.picked_semester.set([picked_semester.id])
                    else:
                        messages.error(request, "Please pick a semester before proceeding")
                        return redirect('profile_view')
